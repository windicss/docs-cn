...
这是一个总结版本，总结了从 v2.1 到 v2.2 的改进和添加的特性。
## 特性 {#features}

- 新插件 [scroll-snap](https://cn.windicss.org/plugins/official/scroll-snap.html#scroll-snap)
...
- 支持排除配置
  ...
  > ```
  >
  > 使用上面的配置，像 `bg-hex-1c1c1e p-4.2 m-3.33px` 这样的工具类将不会被编译成 css。你可以通过使用正则表达式来排除任何不想使用的工具类。
